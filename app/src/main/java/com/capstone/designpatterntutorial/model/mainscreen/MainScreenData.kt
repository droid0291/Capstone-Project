 ...
 */
class MainScreenData : Parcelable {
    private var categoryList: ArrayList<Category?>? = null

    constructor() {}
    protected constructor(`in`: Parcel?) {
        categoryList = `in`.createTypedArrayList(Category.Companion.CREATOR)
    }
    ...
    companion object {
        ...
        val CREATOR: Creator<MainScreenData?>? = object : Creator<MainScreenData?> {
            override fun createFromParcel(`in`: Parcel?): MainScreenData? {
                return MainScreenData(`in`)
            }

            override fun newArray(size: Int): Array<MainScreenData?>? {
                return arrayOfNulls<MainScreenData?>(size)
            }
        }
    }
}