 ...
 */
class NavigationMenuAdapter(private val menuList: Array<String?>?) : RecyclerView.Adapter<NavigationMenuAdapter.DetailsViewHolder?>() {
    ...
    inner class DetailsViewHolder(itemView: View?) : RecyclerView.ViewHolder(itemView) {
        ...
        var selectedMenu: View? = null
        fun bindData(menuTitle: String?, position: Int) {
            title.setText(menuTitle)
            if (selectedItem == position) selectedMenu.setVisibility(View.VISIBLE) else selectedMenu.setVisibility(View.GONE)
