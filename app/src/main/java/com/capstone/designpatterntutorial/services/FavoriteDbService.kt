 ...
 */
class FavoriteDbService : IntentService {
    /**
    ...
    override fun onHandleIntent(intent: Intent?) {
        val operation = intent.getIntExtra(OPERATION_FAVORITE, 0)
        when (operation) {
            OPERATION_ADD -> insertFavoritePattern(intent.getParcelableExtra<Parcelable?>(PATTERN) as Pattern?)
            OPERATION_REMOVE -> deleteFavoritePattern(intent.getParcelableExtra<Parcelable?>(PATTERN) as Pattern?)
            else -> Log.d(TAG, "Invalid Operation Received")
        }
    }
    ...
    private fun deleteFavoritePattern(pattern: Pattern?) {
        ...
        val selection = FavoritePatternEntry.COLUMN_NAME + "=?"
        val selectionArgs = arrayOfNulls<String?>(1)
        selectionArgs[0] = "" + pattern.name
        val result = applicationContext.contentResolver.delete(FavoritePatternEntry.CONTENT_URI, selection, selectionArgs)
        Log.d(TAG, " Deleted Movie with Id:: " + pattern.name + " result:: " + result)
    }

    private fun insertFavoritePattern(pattern: Pattern?) {
        if (pattern == null ||
                MainScreenConverter.isFavoritePattern(applicationContext.contentResolver, pattern.name)) {
            return
        }
        applicationContext.contentResolver.insert(FavoritePatternEntry.CONTENT_URI,
                MainScreenConverter.toFavoritePatternContentValues(pattern))
    }

    companion object {
        private val TAG = FavoriteDbService::class.java.simpleName
        const val OPERATION_ADD = 101
        const val OPERATION_REMOVE = 102
        val OPERATION_FAVORITE: String? = "favoriteOperation"
